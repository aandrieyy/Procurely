...
class Report_model extends CI_Model{
  ...
  public function budget_stats(){
    return $this->db->query("SELECT  

    (SELECT IFNULL(SUM(amount), 0) FROM annual_budget WHERE deleted_at is NULL) as total_annual_budget,
    (SELECT IFNULL(SUM(amount), 0) FROM sector_budget WHERE deleted_at is NULL) as total_sector_budget,
    (SELECT IFNULL(SUM(funds), 0) FROM college_budget WHERE deleted_at is NULL) as college_budget,
    (SELECT IFNULL(SUM(total), 0)  FROM purchase_requests WHERE status = 1 AND deleted_at is NULL) as allocated_pr,
    ((SELECT IFNULL(SUM(amount), 0) FROM annual_budget WHERE deleted_at is NULL) - (SELECT IFNULL(IFNULL(SUM(amount), 0), 0)  FROM sector_budget WHERE deleted_at is NULL)) as remaining_annual_budget,
    ((SELECT IFNULL(SUM(amount), 0) FROM sector_budget WHERE deleted_at is NULL) - (SELECT IFNULL(SUM(funds), 0)  FROM college_budget WHERE deleted_at is NULL)) as remaining_sector_budget,
    ((SELECT IFNULL(SUM(funds), 0) FROM college_budget WHERE deleted_at is NULL) - (SELECT IFNULL(SUM(total), 0)  FROM purchase_requests WHERE status = 1 AND deleted_at is NULL)) as remaining_college_budget
